...
The core flow is: type or speak a generic situation, let Miller decompose its practical needs, review a short source-backed access pathway and verified resources, select resources, then share by Mail, Messages, another iOS share destination, or AirPrint. “Broaden nearby” is explicit: directly serving regional services may appear immediately, while unrelated same-province options are added only when the worker asks.
