...
export default function ServiceMap({ resources, handout, dispatchHandout, onBack, onGetThere, isAdminMode = false, millerAvatar, sessionId }) {
  ...
  async function submitMapChat(event) {
    ...
    try {
      ...
      setChatStatus(contract.noResults || !authorized.length ? "empty" : "complete"); setResultsDismissed(false); setActivePanel("results")
  ...
  function startOver() { setChatInput(""); setChatMessages([]); setChatAnswer(""); setChatResults([]); setChatStatus("idle"); setChatError(""); setResultsDismissed(false); setSelected(null); setSelectedLocation([]); setActivePanel("filters") }
  ...
  return <div className="map-page">
    ...
    <div className="map-workspace">
      ...
      <aside className="map-sidebar" aria-label="Map results and filters"><div className="map-panel-tabs" role="tablist" aria-label="Map sidebar"><button type="button" role="tab" aria-selected={activePanel === "results" && !resultsDismissed} onClick={() => { setResultsDismissed(false); setActivePanel("results") }}>Search results{chatResults.length ? ` (${chatResults.length})` : ""}</button><button type="button" role="tab" aria-selected={activePanel === "filters" || resultsDismissed} onClick={() => setActivePanel("filters")}>Filters</button></div><div role="tabpanel">{activePanel === "results" && !resultsDismissed ? resultsPanel : filtersPanel}</div></aside>
