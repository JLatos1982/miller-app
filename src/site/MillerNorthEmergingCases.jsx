...
export default function MillerNorthEmergingCases() {
  ...
  const visible = useMemo(() => province === "all" ? projection.items : projection.items.filter(item => item.province === province), [province])
  return <main className="mn-public-page mne-page">
    ...
    <section className="mne-grid" aria-label="Developing Research and Policy cases">{visible.map(item => <article className="mne-card" key={item.emerging_case_id}><div className="mne-card-meta"><span>{provinceLabels[item.province]}</span><span>{item.display_state}</span></div><h2>{item.title}</h2><p>{item.description}</p><h3>Why Miller North is watching</h3><p>{item.why_watching}</p><dl><dt>Research stage</dt><dd>{stageLabels[item.stage]}</dd><dt>Latest source check</dt><dd>{item.latest_update_date}</dd></dl><aside><strong>What we are waiting to learn</strong><p>{item.next_question}</p></aside>{item.related_href ? <a className="mne-related" href={item.related_href}>Related mature case →</a> : null}<details><summary>Sources ({item.sources.length})</summary><SourceLinks sources={item.sources}/></details></article>)}</section>
