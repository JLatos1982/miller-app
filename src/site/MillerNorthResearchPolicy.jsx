...
const statusLabels = { implemented: "Implemented", substantially_implemented: "Substantially implemented", partially_implemented: "Partially implemented", implementation_underway: "Implementation underway", implementation_evidence_fragmentary: "Fragmentary evidence" }
...
function RecommendationTracker({ record }) {
  ...
  return <section className="mnrp-recommendations"><div className="mnrp-section-heading"><div><p className="mnrp-kicker">Recommendation tracker</p><h2>What was recommended—and what followed</h2></div><p>{shown.length} of 24 shown</p></div><div className="mnrp-recommendation-filters"><label>Theme<select value={theme} onChange={event => setTheme(event.target.value)}><option value="all">All themes</option>{record.themes.map(item => <option key={item.title}>{item.title}</option>)}</select></label><label>Evidence status<select value={status} onChange={event => setStatus(event.target.value)}><option value="all">All statuses</option>{statusOrder.map(value => <option key={value} value={value}>{statusLabels[value]}</option>)}</select></label></div><div className="mnrp-recommendation-list">{shown.map(item => <details className="mnrp-recommendation" key={item.number}><summary><span className="mnrp-rec-number">{String(item.number).padStart(2, "0")}</span><span><strong>{item.summary}</strong><small>{item.theme}</small></span><span className={`mnrp-status-pill is-${item.status}`}>{item.status_label}</span></summary><div className="mnrp-rec-detail"><section><h3>Responsible organizations</h3><ul>{item.responsible_organizations.map(value => <li key={value}>{value}</li>)}</ul></section><section><h3>What followed</h3>{item.what_followed.length ? <ol>{item.what_followed.map((action, index) => <li key={index}><strong>{action.date}</strong><p>{action.description}</p><small>{action.depth}</small><Sources sources={action.sources} inline /></li>)}</ol> : <p>No discrete action was identified in the sources reviewed.</p>}</section><section><h3>Current public-evidence status</h3><p><span className={`mnrp-status-pill is-${item.status}`}>{item.status_label}</span> · latest evidence {item.latest_evidence_date}</p>{item.evidence_summary.map(value => <p key={value}>{value}</p>)}</section><section className="mnrp-reporting-limit"><h3>Reporting limitation</h3><p>{item.reporting_limitation}</p>{item.source_note && <p><strong>Source note:</strong> {item.source_note}</p>}</section><section><h3>Sources</h3><Sources sources={item.sources} /></section></div></details>)}</div></section>
}

function Themes({ record }) {
  return <section className="mnrp-theme-section"><p className="mnrp-kicker">Thematic view</p><h2>Eight connected areas of change</h2><div className="mnrp-theme-grid">{record.themes.map(theme => <article key={theme.title}><h3>{theme.title}</h3><p><strong>Recommendations:</strong> {theme.recommendation_numbers.join(", ")}</p><div className="mnrp-theme-statuses">{Object.entries(theme.status_distribution).map(([status, count]) => <span className={`is-${status}`} key={status}>{count} {statusLabels[status]?.toLowerCase()}</span>)}</div><p><strong>Evidence:</strong> {theme.strongest_evidence}</p><p><strong>Open question:</strong> {theme.reporting_gap}</p></article>)}</div></section>
}
...
function CaseView({ record, onBack }) {
  return <><button type="button" className="mnrp-back-cases" onClick={onBack}>← All provincial cases</button><header className="mnrp-case-hero"><p className="mnrp-province">{record.province}</p><h1>{record.title}</h1><p>{record.focus}</p><div className="mnrp-hero-metrics">{record.metrics.map(metric => <span key={metric.label}><strong>{metric.value}</strong>{metric.label}</span>)}</div></header><section className="mnrp-significance"><div><p className="mnrp-kicker">What happened</p><p>{record.what_happened}</p></div><div><p className="mnrp-kicker">Why this matters</p><ul>{record.why_this_matters.map(item => <li key={item}>{item}</li>)}</ul></div></section>{record.selection_note && <aside className="mnrp-selection-note"><strong>Why this Alberta case?</strong><p>{record.selection_note}</p></aside>}{record.status_counts && <StatusSummary record={record}/>}<EvidencePath record={record}/>{record.recommendations && <><Themes record={record}/><RecommendationTracker record={record}/></>}<Timeline record={record}/><PolicyContext record={record}/><Findings record={record}/><section className="mnrp-next-question"><p className="mnrp-kicker">Next research question</p><h2>{record.recommended_next_question}</h2><p><strong>The one thing to remember:</strong> {record.one_thing_to_remember}</p></section><SourceLibrary record={record}/><MethodNote/></>
}
...
export default function MillerNorthResearchPolicy() {
  ...
  const [selectedSlug, setSelectedSlug] = useState(() => research.cases.some(item => item.slug === queryCase) ? queryCase : "")
  const selected = research.cases.find(item => item.slug === selectedSlug)
