...
const cases = [...research.cases, fnho]
...
function CaseView({ record, onBack }) {
  return <><button type="button" className="mnrp-back-cases" onClick={onBack}>← All provincial cases</button><header className="mnrp-case-hero"><p className="mnrp-province">{record.province}</p><h1>{record.title}</h1><p>{record.focus}</p><div className="mnrp-hero-metrics">{record.metrics.map(metric => <span key={metric.label}><strong>{metric.value}</strong>{metric.label}</span>)}</div></header><section className="mnrp-opening-summary" aria-label="Case at a glance"><article><p className="mnrp-kicker">What is this?</p><p>{record.what_happened}</p></article><article><p className="mnrp-kicker">Why this matters</p><p>{record.why_this_matters[0]}</p></article><article><p className="mnrp-kicker">What changed afterward?</p><p>{record.what_public_evidence_shows[1]?.text || record.what_public_evidence_shows[0].text}</p></article><article><p className="mnrp-kicker">What remains unclear?</p><p>{record.what_remains_unclear[0].text}</p></article></section>{record.selection_note && <aside className="mnrp-selection-note"><strong>Why this Alberta case?</strong><p>{record.selection_note}</p></aside>}{record.status_counts && <StatusSummary record={record}/>}<EvidencePath record={record}/>{record.recommendations && <><Themes record={record}/><RecommendationTracker record={record}/></>}<Timeline record={record}/><PolicyContext record={record}/><Findings record={record}/><section className="mnrp-next-question"><p className="mnrp-kicker">Next research question</p><h2>{record.recommended_next_question}</h2><p><strong>The one thing to remember:</strong> {record.one_thing_to_remember}</p></section><SourceLibrary record={record}/><MethodNote/></>
}
