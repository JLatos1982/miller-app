...
export default function MillerFundingAssistance({ millerNorth = false }) {
  ...
  const visible = dataset.records.filter(record => (province === "all" || record.jurisdiction === province) && (status === "all" || record.status === status) && (applicant === "all" || record.applicant_types.includes(applicant)))
  ...
  return <main className={millerNorth ? "mn-public-page funding-page funding-page-north" : "funding-page"}>
    ...
    <section className={millerNorth ? "mn-public-hero" : "practical-page-hero"}><p className={millerNorth ? "mn-public-eyebrow" : "practical-eyebrow"}>{millerNorth ? "Miller North" : "Miller"} · Funding &amp; Assistance</p><h1>{millerNorth ? "First Nations and Indigenous funding navigation" : "Financial help for practical next steps"}</h1><p>{millerNorth ? "Current and recurring public opportunities for individuals, Nations, Indigenous organizations and Indigenous businesses." : "Benefits, training, transportation and practical assistance relevant to stability and access to care."}</p></section>
