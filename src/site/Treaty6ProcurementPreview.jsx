...
export default function Treaty6ProcurementPreview() {
  ...
  return <main className="mn-public-page t6p-page">
    ...
    <section className="t6p-section" aria-labelledby="t6p-pathways"><p className="t6p-kicker">Beyond formal tender boards</p><h2 id="t6p-pathways">Ways businesses can find opportunities</h2><p>These are public pathways and marketplace descriptions, not endorsements or confirmed opportunities. Account-only and membership material is never accessed or republished.</p><div className="t6p-grid">{model.sections.contract_pathways.map(item => <article className="t6p-card" key={item.title}><span className="t6p-mini-label">{pretty(item.type)} · {pretty(item.access)}</span><h3>{item.title}</h3><p><strong>May matter for:</strong> {item.relevance}</p><p>{item.summary}</p><OfficialLink href={item.source_url}>Open official pathway</OfficialLink></article>)}</div></section>
