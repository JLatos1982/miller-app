...
function BusinessWatchlistCard({ business }) {
  return <article className="t6p-card t6p-business-card">
    ...
    <p className="t6p-business-disclaimer">Confirm full requirements with the official source before deciding whether to pursue an item.</p>
...
export default function Treaty6ProcurementPreview() {
  ...
  const pathways = [...model.sections.supports, ...model.sections.portals, ...model.sections.indigenous_procurement.map(item => ({ ...item, support_type: "INDIGENOUS_PROCUREMENT" }))]
  ...
  return <main className="mn-public-page t6p-page">
    ...
    <section className="t6p-section" aria-labelledby="t6p-pathways"><p className="t6p-kicker">One place for supplier paths</p><h2 id="t6p-pathways">Ways to find work</h2><p>Official tender boards, supplier programs, marketplaces and Indigenous procurement paths. Account-only and membership material is never accessed or republished.</p><div className="t6p-grid">{pathways.map(item => <SupportCard key={item.support_id || item.portal_id || item.title} item={item} linkText="Open official pathway"/>)}{model.sections.contract_pathways.map(item => <article className="t6p-card" key={item.title}><span className="t6p-mini-label">{pretty(item.type)} · {pretty(item.access)}</span><h3>{item.title}</h3><p>{item.summary}</p><OfficialLink href={item.source_url}>Open official pathway</OfficialLink></article>)}</div></section>
    <section className="t6p-section" aria-labelledby="t6p-buyers"><p className="t6p-kicker">Public purchasing pathways</p><h2 id="t6p-buyers">Buyers & organizations to watch</h2><div className="t6p-grid">{model.sections.buyer_intelligence.map(item => <article className="t6p-card" key={item.name}><span className="t6p-mini-label">{item.jurisdiction}</span><h3>{item.name}</h3><p><strong>Categories:</strong> {item.categories.join(", ")}</p><p>{item.pathway}</p><OfficialLink href={item.source_url}>Official source</OfficialLink></article>)}</div></section>
