...
export default function MillerNorthSeriousHarm() {
  return <main className="mn-public-page mnsh-page">
    <header className="mn-public-header"><MillerNorthHomeLink/><MillerNorthPublicNav current="incidents"/></header>
    <section className="mn-public-hero"><p className="mn-public-eyebrow">Incidents &amp; official records</p><h1>Reviewed healthcare incidents</h1><p>Restrained summaries of individual events for which a coroner, regulator, inquest or institution has placed meaningful evidence on the public record.</p></section>
    ...
    <section className="mnsh-grid" aria-label={`${data.incidents.length} reviewed serious-harm records`}>
      {data.incidents.map(incident => <article id={incident.public_incident_id} key={incident.public_incident_id}>
        ...
        {incident.related_watch_chain_id ? <a className="mnsh-related" href={`/indigenous-healthcare-evidence/accountability-watch#${incident.related_watch_chain_id}`}>Tracked in Accountability Watch →</a> : null}
