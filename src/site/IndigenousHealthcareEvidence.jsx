...
import { evidenceDisplayTitle } from "./millerNorthDisplayTitles.js"
...
export default function IndigenousHealthcareEvidence() {
  ...
  const submitEvidenceSearch = async (event) => {
    ...
    try {
      const response = await fetch("/api/indigenous-healthcare-evidence/search", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query, result_types: searchType === "all" ? [] : searchType.split(","), provinces: searchProvince === "all" ? [] : [searchProvince] }) })
  ...
  return <main className="ihe-page" aria-labelledby="ihe-title">
    ...
    <section className="ihe-search" aria-labelledby="ihe-search-title"><h2 id="ihe-search-title">Search Miller North</h2><p className="ihe-search-intro">Search approved public incidents, evidence, organizations, reports and accountability work.</p><form onSubmit={submitEvidenceSearch}><label className="ihe-search-label" htmlFor="ihe-evidence-query">Search Miller North’s reviewed public material</label><div><input id="ihe-evidence-query" type="search" value={searchQuery} onChange={event => setSearchQuery(event.target.value)} maxLength="500" placeholder="Type here" disabled={searchState === "loading"} /><button type="submit" disabled={!searchQuery.trim() || searchState === "loading"}>{searchState === "loading" ? "Searching…" : "Search"}</button>{evidenceSearch && <button className="ihe-clear-search" type="button" onClick={clearEvidenceSearch}>Clear</button>}</div><div className="ihe-search-filters"><label>Result type<select value={searchType} onChange={event => setSearchType(event.target.value)}>{SEARCH_TYPE_OPTIONS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>Geography<select value={searchProvince} onChange={event => setSearchProvince(event.target.value)}><option value="all">All geographies</option><option value="british_columbia">British Columbia</option><option value="alberta">Alberta</option><option value="saskatchewan">Saskatchewan</option><option value="canada">Canada-wide</option></select></label></div></form><div className="ihe-search-examples" aria-label="Example searches"><span>Try:</span>{EVIDENCE_SEARCH_EXAMPLES.map(example => <button type="button" key={example} onClick={() => setSearchQuery(example)} disabled={searchState === "loading"}>{example}</button>)}</div>{evidenceSearch && <div className={`ihe-search-answer ${searchState === "error" || searchState === "fallback" ? "ihe-search-unavailable" : ""}`} aria-live="polite"><p className="ihe-search-match-count">{evidenceSearch.match_count || 0} relevant result{evidenceSearch.match_count === 1 ? "" : "s"}</p>{evidenceSearch.answer.split(/\n{2,}/).map((paragraph, index) => <p key={index}>{paragraph}</p>)}{evidenceSearch.results?.length ? <ol className="ihe-site-results">{evidenceSearch.results.map(result => <li key={result.result_id}><a href={result.destination_route}><span className="ihe-result-type">{SEARCH_TYPE_LABELS[result.result_type] || result.result_type}</span><strong>{result.title}</strong><span>{result.description}</span><small>{[provinceLabels[result.province] || result.province, result.organization, result.setting, result.date].filter(Boolean).join(" · ")}</small></a></li>)}</ol> : <p className="ihe-search-empty">Try a province, care setting, organization, report title, or accountability topic.</p>}</div>}</section>
    ...
    <section className="ihe-controls" aria-label="Evidence filters">
      <label>Evidence geography<select value={province} onChange={event => setProvince(event.target.value)}><option value="all">All geographies</option>{options.provinces.map(value => <option value={value} key={value}>{provinceLabels[value] || value}</option>)}</select></label>
    ...
    {selected && <section className="ihe-detail" role="dialog" aria-modal="true" aria-labelledby="ihe-detail-title"><div><button className="ihe-close" type="button" onClick={closeSelected} aria-label="Close evidence detail" autoFocus>×</button><p className="ihe-badge">{labels[selected.evidence_status]}</p><h2 id="ihe-detail-title">{evidenceDisplayTitle(selected)}</h2><p className="ihe-detail-note">{selected.evidence_type} · {provinceLabels[selected.province] || selected.province} · {visibleDate(selected)}</p><p>{selected.summary}</p>{selected.caution && <p className="ihe-detail-note"><strong>Context:</strong> {selected.caution}</p>}{selected.recommendation_action && <p><strong>Related action:</strong> {selected.recommendation_action}</p>}<h3>{selected.source_count === 1 ? "Source" : `${selected.source_count} sources`}</h3><ul className="ihe-detail-sources">{selected.sources.map(source => <li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.title}</a><span>{sourceText(source)}</span></li>)}</ul><p className="ihe-detail-note">This is a concise public presentation of {selected.source_record_count} approved source-derived record{selected.source_record_count === 1 ? "" : "s"}; it does not replace the underlying sources.</p></div></section>}
  </main>
}
