...
export default function IndigenousHealthcareEvidence() {
  ...
  const displayedRecords = useMemo(() => {
    if (!evidenceSearch?.record_ids?.length) return filteredRecords
    const byId = new Map(filteredRecords.map(record => [record.public_record_id, record]))
    ...
    return [...retrieved, ...filteredRecords.filter(record => !retrievedIds.has(record.public_record_id))]
  }, [filteredRecords, evidenceSearch])
  ...
  return <main className="ihe-page" aria-labelledby="ihe-title">
    ...
    <section className="ihe-search" aria-labelledby="ihe-search-title"><h2 id="ihe-search-title">Ask the evidence library</h2><form onSubmit={submitEvidenceSearch}><label className="ihe-search-label" htmlFor="ihe-evidence-query">Ask a question about the approved public evidence</label><div><input id="ihe-evidence-query" value={searchQuery} onChange={event => setSearchQuery(event.target.value)} maxLength="500" placeholder="Ask about the evidence..." disabled={searchState === "loading"} /><button type="submit" disabled={!searchQuery.trim() || searchState === "loading"}>{searchState === "loading" ? "Searching…" : "Search"}</button>{evidenceSearch && <button className="ihe-clear-search" type="button" onClick={clearEvidenceSearch}>Clear search</button>}</div></form>{evidenceSearch && <div className={`ihe-search-answer ${searchState === "error" || searchState === "fallback" ? "ihe-search-unavailable" : ""}`} aria-live="polite"><p className="ihe-search-match-count">{evidenceSearch.match_count} locally matched approved public record{evidenceSearch.match_count === 1 ? "" : "s"}</p>{evidenceSearch.answer.split(/\n{2,}/).map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div>}</section>
