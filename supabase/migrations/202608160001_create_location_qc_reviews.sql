...
create table if not exists public.location_qc_reviews (
  canonical_resource_id uuid primary key references public.resource_registry(id),
  policy_version text not null,
  classification_fingerprint text not null,
  decision text not null check (decision in ('pilot_eligible','manual_review','correct_address','exclude_exact_location','policy_problem','defer')),
...
create table if not exists public.location_qc_review_audit (
  ...
  canonical_resource_id uuid not null references public.resource_registry(id),
...
create index if not exists location_qc_review_audit_resource_idx on public.location_qc_review_audit (canonical_resource_id, created_at desc);
...
revoke all on table public.location_qc_reviews, public.location_qc_review_audit from anon, authenticated;
...
create or replace function public.prevent_location_qc_audit_mutation() returns trigger language plpgsql as $$ begin raise exception 'location QC audit is append-only'; end $$;
drop trigger if exists location_qc_audit_append_only on public.location_qc_review_audit;
create trigger location_qc_audit_append_only before update or delete on public.location_qc_review_audit for each row execute function public.prevent_location_qc_audit_mutation();
