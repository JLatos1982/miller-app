...
-- A unit-prefixed Canadian civic address such as "103-12827 76 Ave" is still
-- a complete public civic address.  Preserve the original string everywhere;
-- this only recognizes its numbered civic portion for the existing policy
-- completeness check.  The exact BC-geocoder, occupancy, safety and QC gates
...
begin
  ...
  if coalesce(v_snapshot->>'submitted_address','') !~ '^[[:space:]]*(?:[0-9]+[[:alpha:]]?[[:space:]]*[-–—][[:space:]]*)?[0-9]+[[:alpha:]]?[[:space:]]+.+' or coalesce(v_snapshot->>'returned_address','')='' then return jsonb_build_object('decision','manual_review','reason_code','incomplete_or_nonphysical_address'); end if;
