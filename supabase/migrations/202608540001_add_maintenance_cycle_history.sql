...
-- Keep historical identities deterministic. A volatile default here would be
-- evaluated for existing rows before this backfill and would silently prevent
-- the legacy key derivation below from running.
...
alter table public.miller_maintenance_cycles alter column cycle_key set default encode(extensions.digest(gen_random_uuid()::text,'sha256'),'hex'),alter column cycle_key set not null,alter column trigger_type set not null,alter column phase set not null,alter column completeness set not null;
