...
import { persistSecurityReview, persistSecurityInstrumentFinding, resolveSecurityInstrumentFinding } from "./server/securityFindings.js"
import { pulseFreshness, runSecurityPulse } from "./server/securityPulse.js"
import { createPulseRunStore } from "./server/securityPulseRuns.js"
import { localMillerProfile, MILLER_SECURITY_PROFILE } from "./server/millerSecurityProfile.js"
import { SECURITY_INSTRUMENTS } from "./server/securityInstruments.js"
...
import { securityVersionContext } from "./server/securityVersion.js"
import { deploymentAlignment } from "./server/securityDeployment.js"
import { answerControlRoomQuestion, buildControlRoomSummary } from "./server/controlRoomSummary.js"
import { planAutomationWake, runAutomationWake } from "./server/automationScheduler.js"
import { createAutomationSchedulerStore } from "./server/automationSchedulerStore.js"
import { buildDailyReview, buildDeepReview, buildOperationalGuidance, explainOperationalIntent, routeOperationalIntent } from "./server/operationalGuidance.js"
...
function publicBuildIdentity() {
  const version = securityVersionContext({ profile: MILLER_SECURITY_PROFILE })
  return { status: "healthy", version: version.git_sha || version.build_id || "unknown" }
}

function runtimeEnvironmentPosture() {
  return {
    supabase_url: Boolean(process.env.SUPABASE_URL),
    supabase_service_role: Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY),
    local_sentinel_enabled: process.env.NODE_ENV === "production" && process.env.MILLER_EXTERNAL_SENTINEL_LOCAL_ONLY === "true",
    test_observer_configured: process.env.NODE_ENV === "production" && Boolean(process.env.MILLER_EXTERNAL_SENTINEL_OBSERVER_KEY || process.env.MILLER_EXTERNAL_SENTINEL_ACCESS_TOKEN),
    development_auth_bypass: process.env.NODE_ENV === "production" && process.env.MILLER_DEVELOPMENT_AUTH_BYPASS === "true",
  }
}

function runtimeSchemaContract() {
  return { migration_head: process.env.MILLER_SCHEMA_HEAD || null, contract: "miller-security-deployment-contract-v1", capabilities: ["security_core_registry", "security_incident_correlation", "deployment_observation_ledger"], compatible_heads: [] }
}

app.get("/api/health", (_req, res) => res.json(publicBuildIdentity()))
...
async function controlRoomSnapshot() {
 ...
 const [pulseRuns, securityFindings, securityIncidents, externalObservations, deploymentObservations] = await Promise.all([supabase.from("miller_security_pulse_runs").select("*").order("started_at",{ascending:false}).limit(8),supabase.from("miller_security_findings").select("finding_fingerprint,finding_type,severity,lifecycle,recurrence_count,last_observed_at,instrument_id").order("last_observed_at",{ascending:false}).limit(40),supabase.from("miller_security_incidents").select("correlation_key,category,state,severity,last_observed_at").order("last_observed_at",{ascending:false}).limit(20),supabase.from("miller_external_security_observations").select("observation_key,observation_type,status,observed_at").order("observed_at",{ascending:false}).limit(20),supabase.from("miller_security_deployment_observations").select("build_identity,schema_head,alignment_state,reason_codes,observed_at").order("observed_at",{ascending:false}).limit(8)])
 if ([pulseRuns,securityFindings,securityIncidents,externalObservations,deploymentObservations].some((item)=>item.error)) throw new Error("control_room_unavailable")
...
async function controlRoomSummarySnapshot() {
 ...
 const [findings,pulse,incidents,deployment,automationRuns,automationConfig,maintenance,healthSignals,resourceReviews,shelterReviews,locationReviews,attachments,events]=await Promise.all([
  supabase.from("miller_security_findings").select("finding_fingerprint,finding_type,severity,lifecycle,recurrence_count,last_observed_at").order("last_observed_at",{ascending:false}).limit(40),
  supabase.from("miller_security_pulse_runs").select("status,started_at,completed_at").order("started_at",{ascending:false}).limit(1).maybeSingle(),
  supabase.from("miller_security_incidents").select("correlation_key,category,state,severity,last_observed_at").eq("state","active").order("last_observed_at",{ascending:false}).limit(12),
  supabase.from("miller_security_deployment_observations").select("build_identity,schema_head,alignment_state,reason_codes,observed_at").order("observed_at",{ascending:false}).limit(1).maybeSingle(),
  supabase.from("miller_automation_scheduler_runs").select("status,started_at,completed_at,lease_expires_at,failure_code").order("started_at",{ascending:false}).limit(1).maybeSingle(),
  supabase.from("miller_maintenance_scheduler_config").select("enabled").eq("singleton",true).maybeSingle(),
  supabase.from("miller_maintenance_cycle_journal").select("status,started_at,completed_at").order("started_at",{ascending:false}).limit(8),
  supabase.from("miller_sensor_inspections").select("id,outcome,finished_at,started_at").order("finished_at",{ascending:false}).limit(8),
  ...
  supabase.from("site_events").select("id",{count:"exact",head:true}).gte("created_at",since),
 ])
 if([findings,pulse,incidents,deployment,automationRuns,automationConfig,maintenance,healthSignals,resourceReviews,shelterReviews,locationReviews,attachments,events].some((item)=>item.error))throw new Error("control_room_summary_unavailable")
 ...
 const observed=deployment.data||{alignment_state:runtimeDeployment.state,build_identity:runtimeVersion.git_sha||runtimeVersion.build_id||null,observed_at:null}
 const automationEnabled=process.env.NODE_ENV!=="production"&&process.env.MILLER_AUTOMATION_SCHEDULER_LOCAL_ONLY==="true"
 const automationPlan=planAutomationWake({enabled:automationEnabled,lastRun:automationRuns.data||null,lastPulse:pulse.data||null})
...
app.post("/api/integrations/samwise/canonical-field-correction-v1/preview",requireSamwiseStatus,async(req,res)=>{
  try { const request=validateCanonicalCorrectionRequest(req.body); const result=await supabase.rpc("apply_miller_canonical_field_correction_v1",{p_request:request,p_preview:true}); if(result.error)throw result.error; res.setHeader("Cache-Control","no-store"); return res.json(result.data) } catch(error) { return res.status(correctionResultStatus(error)).json({outcome:"rejected",error:"Canonical correction preview was rejected."}) }
})
...
app.get("/api/admin/control-room-summary",requireAdmin,async(_req,res)=>{try{res.setHeader("Cache-Control","private, no-store");return res.json(await controlRoomSummarySnapshot())}catch{return res.status(503).json({error:"Control Room summary is unavailable. No data was changed."})}})
app.post("/api/admin/miller-guide",requireAdmin,async(req,res)=>{const question=String(req.body?.question||"").slice(0,500),selectedContext=["deployment","security","maintenance","resources","shelters","locations","sensors"].includes(req.body?.selected_context?.type)?{type:req.body.selected_context.type}:null,maintenanceIntent=routeMaintenanceIntent(question),intent=maintenanceIntent||routeOperationalIntent(question);try{if(maintenanceIntent){const summary=await maintenanceToolboxSnapshot(),answer=explainMaintenanceIntent(maintenanceIntent,summary);return res.json({intent:maintenanceIntent,text:answer.text,details:answer.details,source:"deterministic"})}const summary=await controlRoomSummarySnapshot();if(intent==="run_security_pulse")return res.json({intent,action_required:"run_security_pulse",text:"Security Pulse runs fixed local checks only. It does not change application security settings.",details:{safe_observation:true},source:"deterministic"});const answer=answerControlRoomQuestion({question,summary,selectedContext});if(intent!=="unsupported"){const guided=explainOperationalIntent(intent,{...summary,changes:{security:{new:0,recurring:0,resolved:0},public_health:{updates:0,stale_sources:0},operations:{degraded:0}},domains:{security:summary.subsystems.find((item)=>item.id==="security")?.state||"unknown",public_health:summary.subsystems.find((item)=>item.id==="sensors")?.state||"unknown"}});return res.json({intent,text:guided.text,details:guided.details,source:"deterministic"})}return res.json({intent:"control_room_summary",text:answer.text,details:answer.details,source:"deterministic"})}catch{return res.status(503).json({error:"Miller's current summary is unavailable. No action was taken."})}})
...
async function localSecurityHttpProbe({method,path,headers={}}={}) { if (process.env.NODE_ENV==="production" || !["GET","TRACE"].includes(method) || !["/","/api/admin/control-room","/api/admin/maintenance-scheduler"].includes(path)) throw new Error("security_probe_denied"); const controller=new AbortController(), timer=setTimeout(()=>controller.abort(),2500); try { const response=await fetch(`http://127.0.0.1:${port}${path}`,{method,headers,signal:controller.signal,redirect:"manual"}), body=String(await response.text()).slice(0,4096); return {status:response.status,headers:Object.fromEntries(response.headers.entries()),body} } finally { clearTimeout(timer) } }
async function persistSecurityIncidents({ profile, version }) { const [findings,external]=await Promise.all([supabase.from("miller_security_findings").select("finding_fingerprint,finding_type,severity,lifecycle").limit(100),supabase.from("miller_external_security_observations").select("observation_key,observation_type,status").order("observed_at",{ascending:false}).limit(100)]);if(findings.error||external.error)throw findings.error||external.error;const incidents=correlateSecurityEvidence({findings:findings.data||[],external:external.data||[]});for(const incident of incidents){const severity=incident.members.some((member)=>(findings.data||[]).find((item)=>item.finding_fingerprint===member.key)?.severity==="critical")?"critical":"high",saved=await supabase.from("miller_security_incidents").upsert({correlation_key:incident.correlation_key,target_id:profile.targetId,category:incident.category,state:incident.state,severity,reason_codes:incident.reason_codes,version_context:version,last_observed_at:new Date().toISOString()},{onConflict:"correlation_key"}).select("id").single();if(saved.error)throw saved.error;const members=incident.members.map((member)=>({incident_id:saved.data.id,source_kind:member.source,source_key:member.key,reason_code:incident.reason_codes[0]||"correlated"}));if(members.length){const inserted=await supabase.from("miller_security_incident_members").upsert(members,{onConflict:"incident_id,source_kind,source_key",ignoreDuplicates:true});if(inserted.error)throw inserted.error}}return incidents }
async function runLocalScheduledSecurityPulse() {
 if (process.env.NODE_ENV === "production") throw new Error("automation_production_denied")
 const serverSource = fs.readFileSync(new URL(import.meta.url), "utf8"), runs = createPulseRunStore(supabase), profile = localMillerProfile({ origin: `http://127.0.0.1:${port}` }), scheduler = await supabase.from("miller_maintenance_scheduler_config").select("enabled").eq("singleton", true).maybeSingle()
 const result = await runSecurityPulse({
  environment: "local", mode: "local", triggerType: "scheduled_automation", profile, serverSource, clientSource: "", runtime: operationsSnapshot().runtime, schema: runtimeSchemaContract(), environmentPosture: runtimeEnvironmentPosture(),
  scheduler: { enabled: scheduler.data?.enabled === true, schema_available: !scheduler.error, heartbeat_freshness: "current" }, httpProbe: localSecurityHttpProbe,
  startRun: () => runs.start({ triggerType: "scheduled_automation", mode: "local_automation" }), finalizeRun: (run, summary) => runs.finalize(run, summary), failRun: (run, summary) => runs.fail(run, summary),
  loadPrevious: async (instrumentId) => { const previous = await supabase.from("miller_security_findings").select("finding_fingerprint,instrument_id,lifecycle").eq("instrument_id", instrumentId).limit(80); if (previous.error) throw previous.error; return previous.data || [] },
  persist: (finding) => persistSecurityInstrumentFinding({ supabase, finding }), reopen: (finding) => persistSecurityInstrumentFinding({ supabase, finding }), resolve: (finding) => resolveSecurityInstrumentFinding({ supabase, instrumentId: finding.instrument_id, fingerprint: finding.finding_fingerprint }),
  persistOutcome: async (outcome) => { const saved = await supabase.from("miller_security_sensor_outcomes").insert(outcome); if (saved.error) throw saved.error },
  persistCapabilities: async (capabilities) => { const saved = await supabase.from("miller_security_capabilities").upsert(capabilities, { onConflict: "target_id,capability_id" }); if (saved.error) throw saved.error },
  persistDeploymentObservation: async (observation) => { const observation_fingerprint = createHash("sha256").update([observation.target_id, observation.build_identity || "unknown", observation.schema_head || "unknown", observation.profile_version, observation.alignment_state].join("|")).digest("hex"), saved = await supabase.from("miller_security_deployment_observations").upsert({ ...observation, observation_fingerprint }, { onConflict: "observation_fingerprint", ignoreDuplicates: true }); if (saved.error) throw saved.error },
 })
 const incidents = await persistSecurityIncidents({ profile, version: result.version })
 return { ...result, incidents }
}
async function runBoundedAutomationWake() {
 const databaseHost = supabaseUrl ? new URL(supabaseUrl).hostname : ""
 if (process.env.NODE_ENV === "production" || process.env.MILLER_AUTOMATION_SCHEDULER_LOCAL_ONLY !== "true" || !["127.0.0.1", "localhost", "::1"].includes(databaseHost)) throw new Error("automation_local_only_denied")
 const automation = createAutomationSchedulerStore(supabase)
 return runAutomationWake({
  enabled: true, store: automation,
  loadState: async () => { const pulseStore = createPulseRunStore(supabase), [runs, pulseRuns] = await Promise.all([automation.recent(), pulseStore.recent(3)]); const failure_streak = (runs || []).findIndex((run) => run.status !== "failed"), pulseFailureStreak = (pulseRuns || []).findIndex((run) => run.status !== "failed"); return { lastRun: runs[0] ? { ...runs[0], failure_streak: failure_streak === -1 ? runs.length : failure_streak } : null, lastPulse: pulseRuns[0] ? { ...pulseRuns[0], failure_streak: pulseFailureStreak === -1 ? pulseRuns.length : pulseFailureStreak } : null } },
  executeSecurityPulse: runLocalScheduledSecurityPulse,
 })
}
app.get("/api/admin/deployment-status",requireAdmin,async(_req,res)=>{try{const [latestPulse,scheduler,latestObservation]=await Promise.all([supabase.from("miller_security_pulse_runs").select("completed_at,status").order("started_at",{ascending:false}).limit(1).maybeSingle(),supabase.from("miller_maintenance_scheduler_config").select("enabled,updated_at").eq("singleton",true).maybeSingle(),supabase.from("miller_security_deployment_observations").select("build_identity,schema_head,alignment_state,reason_codes,observed_at").order("observed_at",{ascending:false}).limit(1).maybeSingle()]);if([latestPulse,scheduler,latestObservation].some((item)=>item.error))throw new Error("deployment_status_unavailable");const version=securityVersionContext({profile:MILLER_SECURITY_PROFILE}),alignment=deploymentAlignment({profile:MILLER_SECURITY_PROFILE,version,schema:runtimeSchemaContract()});res.setHeader("Cache-Control","private, no-store");return res.json({build_identity:version.git_sha||version.build_id||"unknown",application_started_at:applicationStartedAt,environment:process.env.NODE_ENV||"unknown",profile_version:MILLER_SECURITY_PROFILE.version,alignment,pulse_freshness:pulseFreshness(latestPulse.data?.completed_at),scheduler:{enabled:scheduler.data?.enabled===true,known:Boolean(scheduler.data)},latest_observation:latestObservation.data||null})}catch{return res.status(503).json({error:"Deployment status is unavailable. No deployment action was taken."})}})
app.post("/api/admin/security-pulse/run",requireAdmin,async(req,res)=>{if(req.body?.confirm!==true)return res.status(400).json({error:"Explicit Security Pulse confirmation is required."});if(process.env.NODE_ENV==="production")return res.status(403).json({error:"Security Pulse is local-only and not scheduled."});try{const serverSource=fs.readFileSync(new URL(import.meta.url),"utf8"),clientSource="",mode=req.body?.advisory===true?"advisories":"local",runs=createPulseRunStore(supabase),profile=localMillerProfile({origin:`http://127.0.0.1:${port}`}),scheduler=await supabase.from("miller_maintenance_scheduler_config").select("enabled").eq("singleton",true).maybeSingle(),result=await runSecurityPulse({environment:"local",mode,profile,serverSource,clientSource,runtime:operationsSnapshot().runtime,schema:runtimeSchemaContract(),environmentPosture:runtimeEnvironmentPosture(),scheduler:{enabled:scheduler.data?.enabled===true,schema_available:!scheduler.error,heartbeat_freshness:"unknown"},httpProbe:localSecurityHttpProbe,startRun:()=>runs.start(),finalizeRun:(run,summary)=>runs.finalize(run,summary),failRun:(run,summary)=>runs.fail(run,summary),loadPrevious:async(instrumentId)=>{const previous=await supabase.from("miller_security_findings").select("finding_fingerprint,instrument_id,lifecycle").eq("instrument_id",instrumentId).limit(80);if(previous.error)throw previous.error;return previous.data||[]},persist:(finding)=>persistSecurityInstrumentFinding({supabase,finding}),reopen:(finding)=>persistSecurityInstrumentFinding({supabase,finding}),resolve:(finding)=>resolveSecurityInstrumentFinding({supabase,instrumentId:finding.instrument_id,fingerprint:finding.finding_fingerprint}),persistOutcome:async(outcome)=>{const saved=await supabase.from("miller_security_sensor_outcomes").insert(outcome);if(saved.error)throw saved.error},persistCapabilities:async(capabilities)=>{const saved=await supabase.from("miller_security_capabilities").upsert(capabilities,{onConflict:"target_id,capability_id"});if(saved.error)throw saved.error},persistDeploymentObservation:async(observation)=>{const observation_fingerprint=createHash("sha256").update([observation.target_id,observation.build_identity||"unknown",observation.schema_head||"unknown",observation.profile_version,observation.alignment_state].join("|")).digest("hex"),saved=await supabase.from("miller_security_deployment_observations").upsert({...observation,observation_fingerprint},{onConflict:"observation_fingerprint",ignoreDuplicates:true});if(saved.error)throw saved.error}}),incidents=await persistSecurityIncidents({profile,version:result.version});return res.status(result.status==="already_running"?409:200).json({...result,incidents})}catch{return res.status(503).json({error:"Security Pulse failed safely. No consequential action occurred."})}})
app.post("/api/internal/automation-scheduler/tick",async(req,res)=>{const token=String(process.env.MILLER_AUTOMATION_SCHEDULER_TOKEN||""),localSocket=["127.0.0.1","::1","::ffff:127.0.0.1"].includes(req.socket.remoteAddress);if(process.env.NODE_ENV==="production"||!localSocket||!token||req.get("x-miller-automation-token")!==token)return res.status(403).json({error:"Local automation authorization denied."});try{const result=await runBoundedAutomationWake();return res.status(result.status==="already_running"?409:200).json(result)}catch(error){return res.status(503).json({error:"Local automation heartbeat failed safely.",code:String(error?.message||"automation_scheduler_failed").replace(/[^a-z0-9_-]/gi,"_").slice(0,120)})}})
app.get("/api/admin/security-pulse/runs",requireAdmin,async(_req,res)=>{try{if(process.env.NODE_ENV==="production")return res.status(403).json({error:"Security Pulse is local-only and not scheduled."});const runs=await createPulseRunStore(supabase).recent(20);return res.json({runs})}catch{return res.status(503).json({error:"Security Pulse history is unavailable."})}})
