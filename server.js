...
async function controlRoomSnapshot() {
 ...
 const [pulseRuns, securityFindings] = await Promise.all([supabase.from("miller_security_pulse_runs").select("*").order("started_at",{ascending:false}).limit(8),supabase.from("miller_security_findings").select("finding_fingerprint,finding_type,severity,lifecycle,recurrence_count,last_observed_at,instrument_id").order("last_observed_at",{ascending:false}).limit(40)])
 if (pulseRuns.error || securityFindings.error) throw new Error("control_room_unavailable")
