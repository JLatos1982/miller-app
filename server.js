...
import { runSecurityPulse } from "./server/securityPulse.js"
...
async function controlRoomSnapshot() {
 ...
 const [pulseRuns, securityFindings, securityIncidents, externalObservations] = await Promise.all([supabase.from("miller_security_pulse_runs").select("*").order("started_at",{ascending:false}).limit(8),supabase.from("miller_security_findings").select("finding_fingerprint,finding_type,severity,lifecycle,recurrence_count,last_observed_at,instrument_id").order("last_observed_at",{ascending:false}).limit(40),supabase.from("miller_security_incidents").select("correlation_key,category,state,severity,last_observed_at").order("last_observed_at",{ascending:false}).limit(20),supabase.from("miller_external_security_observations").select("observation_key,observation_type,status,observed_at").order("observed_at",{ascending:false}).limit(20)])
 if ([pulseRuns,securityFindings,securityIncidents,externalObservations].some((item)=>item.error)) throw new Error("control_room_unavailable")
