...
async function maintenanceToolboxSnapshot() {
 const [cycles,outcomes,lessons,opportunities]=await Promise.all([supabase.from("miller_maintenance_cycles").select("*").order("started_at",{ascending:false}).limit(8),supabase.from("miller_maintenance_outcomes").select("*").order("completed_at",{ascending:false}).limit(20),supabase.from("miller_learning_records").select("*").order("last_confirmed_at",{ascending:false}).limit(20),supabase.from("miller_growth_opportunities").select("*").order("priority",{ascending:false}).order("last_observed_at",{ascending:false}).limit(40)])
 if([cycles,outcomes,lessons,opportunities].some((item)=>item.error))throw new Error("maintenance_toolbox_unavailable")
 return maintenanceAdminSummary({cycles:cycles.data||[],outcomes:outcomes.data||[],lessons:lessons.data||[],opportunities:opportunities.data||[]})
}
