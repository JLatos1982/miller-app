...
const inventory = JSON.parse(fs.readFileSync(new URL("../data/location-automation-v1.2.1-review.json", import.meta.url), "utf8")), baseline = inventory.records.map((record) => ({ record, result: classifyLocationCandidate(record) })), unresolved = (requestedIds.size ? baseline.filter(({ record }) => requestedIds.has(record.canonical_uuid)) : baseline.filter(({ record, result }) => result.decision !== "do_not_map" && record.submitted_address && record.program_occupancy_confidence !== "supported")).slice(0, limit)
