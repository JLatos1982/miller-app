...
export function buildShelterAutomationReport(candidates = [], options = {}) {
  const items = candidates.map((candidate) => ({ id: candidate.id, name: candidate.name, source_name: candidate.source_name, review_status: candidate.review_status, location_disclosure_status: candidate.location_disclosure_status, ...classifyShelterCandidate(candidate, options) }))
