export const AUTOMATION_POLICY = Object.freeze({ version: "automation-scheduler-v1", heartbeatMs: 15 * 60 * 1000, pulseMs: 6 * 60 * 60 * 1000, pulseTimeoutMs: 30_000, backgroundTimeoutMs: 30_000, pulseFailureBackoffMs: 60 * 60 * 1000, pulseFailureLimit: 2, leaseMs: 5 * 60 * 1000, maxChildren: 1, productionEligible: false })
...
export async function runAutomationWake({ enabled = false, store, loadState, executeSecurityPulse, executeBackgroundWork, now = () => Date.now(), policy = AUTOMATION_POLICY } = {}) {
  ...
  try {
    ...
    // Background work is a bounded in-process consumer of this existing
    // heartbeat, not a second scheduler. Its failures are isolated from the
    // security pulse and are recorded as failed-safe work.
    if (typeof executeBackgroundWork === "function") {
      try { const result = await within(executeBackgroundWork, policy.backgroundTimeoutMs); started.push({ id: "samwise_background_work", status: result?.cycle?.status || result?.status || "completed" }) }
      catch { started.push({ id: "samwise_background_work", status: "failed_safe" }) }
    }
    const degraded = started.some(item => ["failed", "failed_safe", "GLOBAL_SAFETY_STOP"].includes(item.status))
    await store.finish(run, { status: degraded ? "degraded" : "completed", heartbeat_status: plan.posture.state, due_capabilities: plan.due, started_capabilities: started })
    return { status: degraded ? "degraded" : "completed", orientation: plan, children_started: started.length, started }
