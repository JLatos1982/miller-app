...
import { recoverStaleSecurityPulseRun } from "./maintenanceSecurity.js"
...
export async function runMaintenanceCycle({ mode = "observe", store, persistence, snapshot = async () => ({}), findStaleCycle = async () => null, now = () => Date.now(), db = null, actorId = null, loadMachineQcState = null, securityStore = null } = {}) {
  ...
  try {
    ...
    if (selected) {
      ...
      else if (selected.action_id === "recover_stale_security_pulse_run") result = await recoverStaleSecurityPulseRun({ store: securityStore, run: selected.context, now: now() })
