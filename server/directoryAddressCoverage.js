...
export const DIRECTORY_COVERAGE_VERSION = "miller-directory-address-coverage-v1.1.0"
...
export function reconcileAddressEvidence({ registry = [], aliases = [], tavilyResources = [], curatedResources = [], locations = [], claims = [], evidence = [], qcReviews = [], inventory = {}, geocoded = {} } = {}) {
  ...
  const claimsByResource = new Map(), evidenceByClaim = new Map(), qcByResource = new Map()
  ...
  for (const item of qcReviews) {
    const prior = qcByResource.get(item.canonical_resource_id)
    if (!prior || Number(item.version || 0) > Number(prior.version || 0) || (Number(item.version || 0) === Number(prior.version || 0) && String(item.updated_at || "") > String(prior.updated_at || ""))) qcByResource.set(item.canonical_resource_id, item)
  }
  ...
  const reconciled = active.map((canonical) => {
    ...
    const sourceEvidence = [...occupancyEvidence].filter((item) => item.source_url).sort((a, b) => Number(b.source_authority || 0) - Number(a.source_authority || 0) || String(b.retrieved_at || "").localeCompare(String(a.retrieved_at || "")))[0] || null
    const geocoderEvidence = [...occupancyEvidence].filter((item) => item.source_type === "bc_geocoder" && item.extracted_value && typeof item.extracted_value === "object").sort((a, b) => String(b.retrieved_at || "").localeCompare(String(a.retrieved_at || "")))[0] || null
    const shadowGeocode = geocoderEvidence ? { ...geocoderEvidence.extracted_value, returned_address: geocoderEvidence.extracted_value.standardized_address, location_descriptor: geocoderEvidence.extracted_value.location_descriptor, coordinates: geocoderEvidence.extracted_value.coordinates, shared_address_count: 1 } : null
    const currentAddress = normalizeAddress(occupancy?.proposed_value || linked.source.address || old?.proposed_address || "")
    const currentUrl = sourceEvidence?.source_url || linked.source_url
...
export function triageDirectory(reconciliation) {
  ...
  const records = reconciliation.active.map((item) => {
    ...
    else if (item.headquarters_warning) { category = "administrator_review"; recommendation = "research_service_location"; reason_codes = ["headquarters_not_service_location"] }
    else if (item.qc_decision?.decision === "pilot_eligible") { category = "strong_location_candidate"; recommendation = "administrator_review"; reason_codes = ["durable_qc_strong_candidate"] }
    else if (item.qc_decision?.decision === "manual_review") { category = "administrator_review"; recommendation = "administrator_review"; reason_codes = ["durable_qc_manual_review"] }
    ...
    const shadowGeocoder = item.geocode ? { provider: item.geocode.provider, score: item.geocode.score, precision: item.geocode.precision, descriptor: item.geocode.location_descriptor, faults: item.geocode.faults || [] } : null
    const confidence = category === "strong_location_candidate" ? "strong" : category === "administrator_review" ? "needs_review" : existing?.confidence || (recommendation === "excluded" ? "excluded" : category === "probable_fixed_address_candidate" ? "needs_review" : "insufficient")
    return { ...item, category, recommendation, reason_codes, confidence, proposed_address: item.current_address || existing?.proposed_address || "", standardized_address: item.geocode?.returned_address || existing?.standardized_address || null, geocoder: shadowGeocoder || existing?.geocoder || null, concerns: [...reason_codes, ...(item.needs_reevaluation ? ["canonical_evidence_changed"] : [])], public_map: item.mapped }
  })
...
export function buildDirectoryCoverageReport(input) {
  ...
  const strongCategories = new Set(["strong_location_candidate", "high_confidence_candidate", "likely_candidate_needing_review"])
  const realisticallyMappable = triage.records.filter((item) => strongCategories.has(item.category)).length
  const administratorReview = triage.records.filter((item) => item.category === "administrator_review").length
  return { version: DIRECTORY_COVERAGE_VERSION, generated_at: new Date().toISOString(), total_active: reconciliation.active.length, reconciliation: { old_evidence_active: reconciliation.oldEvidenceActive, evidence_reused: reconciliation.reused, evidence_changed_requires_reevaluation: reconciliation.changed, retired_or_replaced_evidence_preserved: reconciliation.retiredEvidence.length, newly_examined_deterministically: reconciliation.newlyExamined }, counts: triage.counts, canonical_counts: { public_locations: triage.counts.already_public_mapped || 0, strong_private_candidates: realisticallyMappable, administrator_review: administratorReview, occupancy_unresolved: triage.counts.geocoder_valid_occupancy_unverified || 0, shared_building_review: triage.counts.shared_building || 0, conflicting: triage.counts.conflicting_addresses || 0, approximate: triage.counts.approximate_geocode || 0, fixed_location_research_leads: triage.counts.probable_fixed_address_candidate || 0 }, meaningfully_evaluated: meaningful, coverage_percentage: reconciliation.active.length ? Number((meaningful / reconciliation.active.length * 100).toFixed(1)) : 0, realistically_mappable_if_approved: realisticallyMappable, administrator_review_candidates: administratorReview, fixed_location_research_leads: triage.records.filter((item) => item.category === "probable_fixed_address_candidate").length, ranked_queue: ranked, seven_candidate_reevaluation: triage.records.filter((item) => item.prior_strong_candidate), records: triage.records, retired_evidence: reconciliation.retiredEvidence, shadow_writes: 0, public_location_changes: 0 }
}
