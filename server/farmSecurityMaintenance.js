...
export function inventoryFarmSecurityMaintenance(root = process.cwd()) {
  ...
  return {
    ...
    systems: [
      ...
      implemented("dependency_advisory", "monthly", "farm_adapter", "activated_read_only", "Runs npm's advisory read path; no automatic package mutation."),
      implemented("worker_transport_health", "each_cycle", "server/farmIgorWorker.js", "activated_read_only", "Authenticates a short-lived local capability handshake; it does not restart or reconfigure workers."),
      ...
      implemented("backup_recovery_check", "owner_scheduled", "reports/farm-igor-listeners-location-security-handoff-2026-09-07.md", "planned_only", "Git-backed registries are recoverable, but no off-host listener-memory copy or supervised Supabase restore drill was verified."),
