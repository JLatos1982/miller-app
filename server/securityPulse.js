...
export async function runSecurityPulse({ environment = "local", mode = "local", repositoryRoot = process.cwd(), serverSource = "", clientSource = "", startRun = async () => ({ id: "ephemeral" }), finalizeRun = async () => {}, failRun = async () => {}, loadPrevious = async () => [], persist = async () => {}, resolve = async () => {}, reopen = async () => {}, repositoryHygiene = inspectRepositoryHygiene } = {}) {
  ...
  try {
    ...
    return { run_id: run.id, status: status === "completed" ? "healthy" : "review_required", completeness: unavailable ? "partial" : "complete", scheduler: "disabled_manual", external_requests: SECURITY_PULSE_MODES[mode].external_requests, mutations: 0, instruments, findings: observations.flatMap((item) => item.findings), delta: runSummary, rhythms: SECURITY_RHYTHMS }
