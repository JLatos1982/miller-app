...
export const SECURITY_PULSE_MODES = Object.freeze({ local: { external_requests: 0, instruments: ["configuration_drift", "repository_hygiene", "http_header_posture", "auth_boundary", "request_boundary", "response_hygiene", "runtime_anomaly", "public_surface", "auth_boundary_variants", "response_contract"] }, advisories: { external_requests: 1, instruments: ["configuration_drift", "repository_hygiene", "dependency_posture"] }, deep_local: { external_requests: 0, instruments: ["configuration_drift", "repository_hygiene", "static_analysis", "container_posture"] } })
...
export async function runSecurityPulse({ environment = "local", mode = "local", profile = MILLER_SECURITY_PROFILE, repositoryRoot = process.cwd(), serverSource = "", clientSource = "", runtime = {}, startRun = async () => ({ id: "ephemeral" }), finalizeRun = async () => {}, failRun = async () => {}, loadPrevious = async () => [], persist = async () => {}, resolve = async () => {}, reopen = async () => {}, persistOutcome = async () => {}, persistCapabilities = async () => {}, repositoryHygiene = inspectRepositoryHygiene, dependencyAudit = runDependencyAdvisoryAudit, httpProbe = null } = {}) {
  ...
  try {
    ...
    const version = securityVersionContext({ profile })
