...
export const SECURITY_PULSE_MODES = Object.freeze({ local: { external_requests: 0, instruments: ["configuration_drift", "repository_hygiene", "http_header_posture", "auth_boundary", "request_boundary", "response_hygiene", "runtime_anomaly"] }, advisories: { external_requests: 1, instruments: ["configuration_drift", "repository_hygiene", "dependency_posture"] }, deep_local: { external_requests: 0, instruments: ["configuration_drift", "repository_hygiene", "static_analysis", "container_posture"] } })
...
export async function runSecurityPulse({ environment = "local", mode = "local", profile = MILLER_SECURITY_PROFILE, repositoryRoot = process.cwd(), serverSource = "", clientSource = "", runtime = {}, startRun = async () => ({ id: "ephemeral" }), finalizeRun = async () => {}, failRun = async () => {}, loadPrevious = async () => [], persist = async () => {}, resolve = async () => {}, reopen = async () => {}, persistOutcome = async () => {}, repositoryHygiene = inspectRepositoryHygiene, dependencyAudit = runDependencyAdvisoryAudit, httpProbe = null } = {}) {
  ...
  try {
    ...
    const runSummary = { ...summary, instruments_attempted: observations.length, instruments_succeeded: observations.length - unavailable, instruments_degraded: unavailable, instruments_unavailable: unavailable, attention_worthy: attention, duration_ms: Date.now() - beganAt, summary: { scheduler: "disabled_manual", external_requests: SECURITY_PULSE_MODES[mode].external_requests, mode } }
    ...
    return { run_id: run.id, target: { id: profile.targetId, profile_version: profile.version }, status: status === "completed" ? "healthy" : "review_required", completeness: unavailable ? "partial" : "complete", scheduler: "disabled_manual", external_requests: SECURITY_PULSE_MODES[mode].external_requests, mutations: 0, instruments, capabilities: capabilityRegistry({ profile, instruments: SECURITY_INSTRUMENTS, outcomes: observations }), capability_gaps: capabilityGaps({ profile, instruments: SECURITY_INSTRUMENTS, outcomes: observations }), findings: observations.flatMap((item) => item.findings), delta: runSummary, rhythms: SECURITY_RHYTHMS }
