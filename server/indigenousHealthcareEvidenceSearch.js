...
export function buildMillerNorthPublicSearchIndex() {
  ...
  for (const item of evidence.groups) {
    ...
    values.push(result({ result_id: `evidence:${item.public_record_id}`, result_type: item.evidence_status === "reported_account" ? "incident" : "evidence", title: item.source?.title || item.evidence_type, description: item.summary, province: item.province, date: item.source?.publication_date || item.year, organization: item.organization || item.source?.publisher, setting, status: item.evidence_status, confidence: item.evidence_status === "formal_finding" ? "high" : null, destination_route: `/indigenous-healthcare-evidence?group=${item.public_record_id}`, underlying_record_id: item.public_record_id, search_text: compact([item, item.sources, setting]).join(" ") }))
  }
  ...
  for (const item of seriousHarm.incidents) values.push(result({ result_id: `serious-harm:${item.public_incident_id}`, result_type: "incident", title: item.title, description: item.summary, province: item.province, date: item.event_date, organization: item.sources?.[0]?.title, setting: item.care_setting, status: item.evidence_strength?.key, confidence: ["formal_process_evidence", "final_formal_outcome"].includes(item.evidence_strength?.key) ? "high" : "reviewed", destination_route: `/indigenous-healthcare-evidence/serious-harm#${item.public_incident_id}`, underlying_record_id: item.public_incident_id, search_text: compact(item).join(" ") }))
