...
export function buildAccessContext({ resource, location, transit, userCoordinate = null }) {
  ...
  return { kind: "miller_access_context", resourceId: String(resource.id), location: approvedLocation, userDistance: Number.isFinite(userDistance) ? { method: "straight_line", kilometres: userDistance } : null, transit: transit ? { provider: transit.provider, nearbyStops: transit.data.stops, relevantAlerts: transit.realtime.alerts, status: transit.realtime.status } : null, provenance: transit?.provenance || null }
}
