...
export const MAP_AUTO_PUBLISH_MODES = Object.freeze({ CLASSIFICATION_ONLY: "classification-only", DETERMINISTIC_REFRESH_DRY_RUN: "deterministic-refresh-dry-run" })
...
export function buildMapAutoPublishContexts({ resources = [], claims = [], evidence = [], qc = [], locations = [] }) {
  ...
  return enumerateMapAutoPublishCandidates({ resources, locations }).map(({ resource, priority }) => {
    const occupancyClaim = newest(claims.filter((claim) => claim.resource_id === resource.id && claim.field_name === "location_occupancy"))
    return { resource, priority, qc: qcByResource.get(resource.id) || null, occupancyClaim, evidence: occupancyClaim ? evidenceByClaim.get(occupancyClaim.id) || [] : [], locations: locations.filter((location) => location.resource_id === resource.id) }
  })
}
...
export async function evaluateMapAutoPublishCandidate({ db, item, mode = MAP_AUTO_PUBLISH_MODES.CLASSIFICATION_ONLY, deterministicRefresh }) {
  ...
  if (!item.occupancyClaim) return { decision: "manual_review", reason_code: "authoritative_occupancy_claim_required", mode, rpc_called: false }
