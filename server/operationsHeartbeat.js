...
export function evaluateOperationalHeartbeat({ databaseReachable = false, security = {}, quietMaintenanceEnabled = false, workingMemory = 0, sensors = [], pulse = null, now = Date.now() } = {}) {
  const pulseAge = pulse?.completed_at ? now - new Date(pulse.completed_at).getTime() : null
  const pulseStatus = !pulse ? "never_run" : pulse.status === "failed" ? "failed" : pulse.status === "degraded" ? "degraded" : pulseAge > 6 * 60 * 60 * 1000 ? "stale" : "current"
  const checks = [
    ...
    { id: "security_pulse", status: pulseStatus === "current" || pulseStatus === "never_run" ? "pass" : "review_needed", detail: pulseStatus === "current" ? "The latest manual Security Pulse is current." : pulseStatus === "never_run" ? "Security Pulse has not been run yet." : `Security Pulse is ${pulseStatus}.` },
  ]
