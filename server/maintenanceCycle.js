...
export function orientMaintenanceCycle({ heartbeat = {}, guidance = {}, planner = {}, pulse = null, sensors = [], previous = null } = {}) {
  const needs = []
  ...
  const ordered = needs.sort((a, b) => priority[a.severity] - priority[b.severity] || a.id.localeCompare(b.id)).slice(0, 20)
  return { phase: "orienting", prior_cycle: previous?.id || null, inventory: { system: heartbeat.status || "unknown", security: guidance.domains?.security || "unknown", public_health: guidance.domains?.public_health || "unknown", pulse: pulse?.status || "never_run" }, needs: ordered, safe_work: ordered.filter((item) => item.tier === 1 && item.executable), human_review: ordered.filter((item) => !item.executable), scheduling: "not_enabled" }
}
