...
export function createMaintenanceCycleStore(supabase, { now = () => Date.now() } = {}) {
  ...
  return { inspectActive: active, async start(mode = "observe") { const current = await active(); if (current) return { already_running: true, cycle: current }; const result = await supabase.from("miller_maintenance_cycles").insert({ cycle_key: key(), mode, trigger_type: "manual_admin", phase: "waking", status: "running", completeness: "partial" }).select().single(); if (result.error) { if (result.error.code === "23505") return { already_running: true, cycle: await active() }; throw result.error } return { already_running: false, cycle: result.data } }, finish, fail(cycle, summary = {}) { return finish(cycle, { ...summary, status: "failed", completeness: "failed" }) } }
}
