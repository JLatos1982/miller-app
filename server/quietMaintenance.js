...
export function buildQuietMaintenancePlan({ runKey, topics = [], signals = [], hypotheses = [], buckets = [], directives = [], asOf = new Date().toISOString() } = {}) {
  ...
  const directiveWeight = (topicKey) => directives.filter((item) => item.status === "active" && item.topic_key === topicKey && dateMs(item.expires_at) > dateMs(asOf)).reduce((sum, item) => sum + (item.directive_type === "de_emphasize" ? -1 : 1) * Number(item.strength || 0), 0)
  const activeTopics = allRegulated.filter(({ next, topic }) => topic.status === "active" && next.current_score >= 15).sort((a, b) => (b.next.current_score + directiveWeight(b.topic.topic_key) * 5) - (a.next.current_score + directiveWeight(a.topic.topic_key) * 5) || a.topic.topic_key.localeCompare(b.topic.topic_key))
  const familyCounts = new Map(), carryTopics = []
  for (const item of activeTopics) { const current = familyCounts.get(item.family) || 0; if (current >= QUIET_MAINTENANCE.maxPerFamily || carryTopics.length >= QUIET_MAINTENANCE.maxCarryForward) continue; familyCounts.set(item.family, current + 1); const human_directive_weight = directiveWeight(item.topic.topic_key); carryTopics.push({ kind: "attention", topic_key: item.topic.topic_key, state: item.next.state, score: item.next.current_score, family: item.family, human_directive_weight, human_directed: human_directive_weight !== 0 }) }
  ...
  const plan = { attention_updates, hypothesis_updates, expired_buckets, integrity_findings, reflections: reflections.slice(0, QUIET_MAINTENANCE.maxReflections), inspected_counts: { attention_topics: topics.length, attention_signals: signals.length, coverage_hypotheses: hypotheses.length, aggregate_buckets: buckets.length, active_directives: directives.filter((item) => item.status === "active").length }, action_counts: { attention_regulated: attention_updates.length, duplicate_signal_suppressed: suppressed, attention_decayed: decayed, hypotheses_resolved: resolved, expired_items_forgotten: expired, safe_repairs: 0, integrity_findings: integrity_findings.length }, carry_forward, result_summary: { external_requests: 0, canonical_mutations: 0, map_mutations: 0, contradictions_found: integrity_findings.length, unresolved_uncertainties: carry_forward.filter((item) => item.kind === "coverage_hypothesis").length + integrity_findings.length, bounded: true, privacy: "aggregate_only", human_directives_rank_attention_only: true } }
