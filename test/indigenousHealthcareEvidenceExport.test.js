...
test("exports use the exact current filtered public record set, including year states", () => {
  assert.equal(filterEvidenceRecords(projection.records, all).length, 695)
  ...
  const canada = filterEvidenceRecords(projection.records, { ...all, province: "canada" })
  assert.equal(canada.length, 14)
  assert.ok(canada.every(record => record.province === "canada"))
  const reported = filterEvidenceRecords(projection.records, { ...all, status: "reported_account" })
  assert.equal(reported.length, 44)
  ...
  assert.equal(unknown.length, 695)
...
test("print source excludes controls and includes the conditional caution and footer", () => {
  ...
  assert.match(page, /Exact event year/)
  assert.match(page, /visibleDate\(record\)/)
