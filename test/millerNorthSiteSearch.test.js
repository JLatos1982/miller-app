...
test("site search handles phrases, aliases, geography, settings and typo tolerance with stable ranking", () => {
  ...
  assert.ok(retrieveMillerNorthResults("Tsekene Prince George custody medical support").some(item => item.underlying_record_id === "mnsh_lindsey_izony_2019"))
  ...
  assert.ok(retrieveMillerNorthResults("Alyssa George Terrace custody withdrawal recommendations").some(item => item.underlying_record_id === "mnsh_alyssa_george_2013"))
  assert.ok(retrieveMillerNorthResults("Jacob Setah youth mental health transfer").some(item => item.underlying_record_id === "mnsh_jacob_setah_2014"))
  ...
  assert.ok(retrieveMillerNorthResults("trauma informed gynecology Indigenous").some(item => item.underlying_record_id === "mnsh_cpsbc_trauma_informed_gynecology_2025"))
  assert.ok(retrieveMillerNorthResults("Metis outpatient psychiatrist dismissed").some(item => item.underlying_record_id === "mnsh_cpsbc_metis_mental_health_2024"))
  assert.ok(retrieveMillerNorthResults("Indigenous diabetes testing generalization").some(item => item.underlying_record_id === "mnsh_cpsbc_primary_care_generalization_2025"))
