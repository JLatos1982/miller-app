...
test("capability diagnostic reports presence without secret values", () => { const report = capabilityReport({ OPENAI_API_KEY: "do-not-expose", SUPABASE_URL: "configured", SUPABASE_SERVICE_ROLE_KEY: "also-secret" }); assert.equal(report.capabilities.find((item) => item.id === "miller_ai").status, "configured"); assert.equal(report.capabilities.find((item) => item.id === "translink_realtime").status, "not_configured"); assert.doesNotMatch(JSON.stringify(report), /do-not-expose|also-secret/) })
