...
test("companion restores public-safe, need-specific safety and readiness guidance", () => {
  ...
  assert.match(buildMillerCompanionResponse({ query: "My brother is using and I don't know what to do", response: base }), /supporting someone else/i)
  assert.match(buildMillerCompanionResponse({ query: "My sister needs counselling", response: { ...base, interpreted: { primary_intent: "counselling" } } }), /supporting someone else/i)
  assert.doesNotMatch(buildMillerCompanionResponse({ query: "I need help talking to my friend about school", response: {} }), /supporting someone else/i)
  const dailyDrinking = buildMillerCompanionResponse({ query: "I'm drinking every day and want to stop", response: { ...base, interpreted: { primary_intent: "treatment" } } })
  assert.match(dailyDrinking, /stopping suddenly can sometimes carry withdrawal risk/i)
  assert.doesNotMatch(dailyDrinking, /urgent medical help/i)
