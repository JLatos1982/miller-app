...
test("approved First Nations Healthcare Evidence projection is public-only and approval-bound", () => {
  ...
  assert.equal(new Set(projection.records.map(record => record.public_record_id)).size, 695)
  assert.equal(projection.records.filter(record => record.evidence_status === "reported_account").length, 44)
  assert.equal(projection.records.filter(record => record.evidence_status === "systemic_evidence").length, 552)
  assert.equal(projection.records.filter(record => record.evidence_status === "official_investigation").length, 91)
  ...
  assert.equal(projection.records.filter(record => record.province === "alberta").length, 188)
  assert.equal(projection.records.filter(record => record.province === "british_columbia").length, 373)
  assert.equal(projection.records.filter(record => record.province === "saskatchewan").length, 120)
  assert.equal(projection.records.filter(record => record.province === "canada").length, 14)
...
test("Miller integrates the hidden Miller North routes and shared public navigation without a Samwise runtime dependency", () => {
  ...
  assert.doesNotMatch(navigation, /Live Listening/)
  assert.match(page, /Search &amp; Evidence/)
  assert.match(page, /<MillerNorthStartHere/)
  assert.doesNotMatch(page, /ihe-layer-context/)
