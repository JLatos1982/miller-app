...
import { askMiller, buildMillerRequest, MILLER_CONTRACT_VERSION, MillerApiError, toMillerResourceMatch } from "../src/millerApi.js"
import { validateMillerRequest } from "../server/millerValidation.js"

const noisy = { id: "curated:abc", name: "Clinic", city: "Burnaby", approved: true, fundingType: "public", latitude: 49, longitude: -123, virtual_service: false, public_map: true, verification_status: "verified" }

test("normal Resource Finder request strips UI and geography fields and passes validation", () => { const payload = buildMillerRequest({ mode: "main", query: "detox", city: "All Cities", matches: [noisy] }); assert.deepEqual(Object.keys(payload.matches[0]).sort(), ["approved", "city", "id", "name"]); assert.equal(validateMillerRequest(payload).interface, "main") })
test("map request passes validation without leaking normal discovery controls", () => { const payload = buildMillerRequest({ mode: "map", query: "OAT", matches: [noisy] }); assert.equal(validateMillerRequest(payload).interface, "map"); assert.equal("tavily" in payload, false) })
test("map-only fields are not required in normal mode", () => { assert.doesNotThrow(() => validateMillerRequest({ query: "virtual counselling", interface: "main" })) })
test("numeric, curated, and canonical IDs survive trusted request serialization", () => { for (const id of [30, "curated:abc", "c590181c-3721-58c6-9027-c2af7dc60108"]) assert.equal(toMillerResourceMatch({ id, name: "Resource" }).id, id) })
